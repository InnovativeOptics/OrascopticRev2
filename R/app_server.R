...
orascoptic_data <- googlesheets4::read_sheet(sheet_id, sheet = "Loupe_types", col_types = "c") %>%
  filter(`Mfg` == 'Orascoptic Current Models') %>%
  rename(`Orascoptic Frame` = Mod, Style = Size, `Innovative Optics Insert` = `Insert Part Number` )
...
#orascoptic_data <- readxl::read_excel("data/Dental_data.xlsx",
#                                      sheet = "Loupe_types") %>%
#  filter(`Mfg` == "Orascoptic Current Models") %>%
#  rename(`Orascoptic Frame` = Mod, Style = Size, `Innovative Optics Insert` = `Insert Part Number`)
...
############################################################################################
###COMMENTED OUT BECAUSE THIS WAS COPIED FROM FIRST VERISION OF ORACOPTIC BEFORE COMBINATION
### OF THREE SHEETS OF DENTAL_DATA FILE
##############################################################################################
...
app_server <- function(input, output, session) {
  ...
  selected_data <- eventReactive(c(input$loupestyle, input$style, input$mod),{
    req(input$mfg)
    result <- dental_data %>%
      filter(`Laser Mfg` == input$mfg,
...
#      mutate(`INVO Part Number` = glue::glue_safe("<a href='{Website}' target ='_blank'> {INVO Part Number Raw} </a> "))
  ...
  user_info <- eventReactive(input$run,{
    result <- tibble(
      "Orascoptic Loupe Style" = loupe_insert()$`Orascoptic Frame`,
  ...
  image_location <- eventReactive(input$run,{
    ...
    result <- c(glue::glue_safe("www/OrascopticLoupeImages/", loupe_rec$LoupeImages[[1]]),
                if_else(selected_data()$`Eyewear Lens Compatible` %in% c("Pi19", "Pi23"),
                        glue::glue_safe("www/recs/", selected_data()$`Rec1`, ".jpeg"),
                        glue::glue_safe("www/recs/", selected_data()$`Rec1`, ".jpg")
                ),
